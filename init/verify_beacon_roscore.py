...
def start_verification():
    ...
    if not network_manager.get_ipv4("wlan0") == config["SELF_IP_ADDRESS_ON_TPLINK"][0]:
        #Condition not satisfied
        print(f"Log [{FILE_NAME}]: Impossible de se connecter au Louise roscore. Vous devez vous connecter"+\
            f" au TP Link afin d'avoir l'addresse IP suivante : {config['SELF_IP_ADDRESS_ON_TPLINK'][0]}")
        return False

    #2nd : Beacon roscore must already run on the TP-Link network
    #Run netcat command to ping beacon specific ROS port
    nc_string_bytes = subprocess.run(["nc","-zv", config["LOUISE_IP_ON_TPLINK"][0], "11311", "-w", "0.6"],
    ...
    if not nc_string.split()[-1] == "succeeded!":
        ...
        print(f"Log [{FILE_NAME}]: Louise roscore non détecté. Vérifiez que Louise à bien lancer"+\
            f" son roscore sur son addresse IP TP Link ({config['LOUISE_IP_ON_TPLINK'][0]}).")
        return False
